...
main = do
    ...
    xmonad $ docks $ewmh def
        ...
        , logHook               = dynamicLogWithPP $ namedScratchpadFilterOutWorkspacePP $ xmobarPP -- need filterout the scratchpad, or it may lead statuebar stuck
                                ...
                                , ppCurrent         = xmobarColor "#000000" "#7e749d" . wrap " " " "           -- Current workspace
                                , ppVisible         = xmobarColor "#ffa500" "" . clickable              -- Visible but not current workspace
                                , ppHidden          = xmobarColor "#82AAFF" "" . wrap "\61552" "" . clickable -- Hidden workspaces
...
-- others
    ...
    , ((mod4Mask, xK_r), spawn "dmenu_run -i -fn 'Source Han Sans CN-10' -sb '#7e749d' -sf '#F0FFFF' -p 'execute     :)'")  --app launcher with dmenu
    , ((mod4Mask, xK_p), spawn "ffcast png ~/Pictures/capture/$(date +%Y%m%d%H%M%S).png && dunstify -t 3000 'screen capture in ~/Pictures/capture'")  --fullscreen capture
...
-- scratchpad key
    ...
    , ((mod4Mask, xK_s), namedScratchpadAction myScratchPads "xpad")
